...
class CalendarWidgetContent extends Component {
    ...
    render() {
        ...
        return (
            <div className={styles.content}>
                ...
                <CalendarRow 
                    event="1. tréning" 
                    time={`${data.acf.fp1_time} - ${format(addMinutes(parse(data.acf.fp1_time, 'HH:mm', new Date()), 90), "HH:mm")}` }
                    tv={data.acf.fp1_tv}/>
                <CalendarRow 
                    event="2. tréning" 
                    time={`${data.acf.fp2_time} - ${format(addMinutes(parse(data.acf.fp2_time, 'HH:mm', new Date()), 90), "HH:mm")}` }
                    tv={data.acf.fp2_tv}/>
                <CalendarRow 
                    event="3. tréning" 
                    time={`${data.acf.fp3_time} - ${format(addMinutes(parse(data.acf.fp3_time, 'HH:mm', new Date()), 60), "HH:mm")}` }
                    tv={data.acf.fp3_tv}/>
                <CalendarRow 
                    event="Kvalifikácia" 
                    time={`${data.acf.q_time} - ${format(addMinutes(parse(data.acf.q_time, 'HH:mm', new Date()), 60), "HH:mm")}` }
                    tv={data.acf.q_tv}/>
                <CalendarRow 
                    event="Preteky" 
