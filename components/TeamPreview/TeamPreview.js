    ...
    return (
      <div className={styles.container}>
        <Link href="timy/[id]" as={`/timy/${constructor.slug}`}>
          <a className={`noOutline`} style={{ marginBottom: "5px" }}>
            <span className={styles.teamName}>{constructor.name}</span>
          </a>
        </Link>
        <Link href="timy/[id]" as={`/timy/${constructor.slug}`}>
          <a
        ...
        <div className={styles.driverRow}>
          {constructor.Drivers.map(driver => {
            return (
              ...
              >
                <div
                  className={`noOutline ${styles.driverBoxLine}`}
                  style={{ borderColor: constructor.teamColor }}
                >
                  <div className={`${styles.driverBox}`}>
                    <a
                      className={`${styles.name} ${styles.slider} ${getCName(
                        constructor.name
                      )}`}
