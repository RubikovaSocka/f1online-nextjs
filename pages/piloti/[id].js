...
export default class DriverPage extends Component {
    ...
    render() {
        let driverBioData, driverPosts;
        let driver = this.props.driverData

        driverBioData = (
            <Fragment>
                <SectionTitle title={`${driver.givenName} ${driver.familyName}`}/>
                <Divider height='20px' />
                <div className={styles.briefInfoContainer}>
                    <img className={styles.driverPortrait} alt={`Portrét pilota ${driver.givenName} ${driver.familyName}`} src={driver.img800} />
                    <div className={styles.tableContainer}>
                        {<div className={styles.numberContainer}>
                            <img alt={`Číslo ${driver.permanentNumber}`} src={`https://wpadmin.f1online.sk/wp-content/uploads/${driver.permanentNumber}.jpg`}/> 
                        </div>}
                        <div className={styles.table}>
                            ...
                            <div className={styles.row}>
                                <span>Body</span>
                                <span>{`${driver.stats.points}`}</span>
                            </div>
                            <div className={styles.row}>
                                <span>Narodený</span>
                                <span>{`${driver.dateOfBirth.split('-')[2]}. ${driver.dateOfBirth.split('-')[1]}. ${driver.dateOfBirth.split('-')[0]}, ${driver.town}`}</span>
                            </div>
                        </div>
                    </div>
                </div>
